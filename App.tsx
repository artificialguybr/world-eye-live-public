    ...
    <div className="relative h-screen w-screen bg-black text-white overflow-hidden font-sans selection:bg-white/20">
      ...
      >
        ...
        <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-start z-50 pointer-events-none">
          <div className={`flex gap-4 ${viewMode === 'immersive' ? 'pointer-events-auto' : ''}`}>
            {/* Back to Map Button */}
            <button
              onClick={() => setViewMode('map')}
              className="glass px-4 py-2 rounded-full flex items-center gap-2 hover:bg-white/10 transition-colors group touch-manipulation"
            >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 pb-10 md:p-8 md:pb-10 z-50 flex flex-col items-center md:flex-row justify-between items-end gap-6 pointer-events-none">
          ...
          {/* Left: Location Info */}
          <div className={`max-w-md transition-transform duration-500 hover:scale-[1.02] mb-20 md:mb-0 ${viewMode === 'immersive' ? 'pointer-events-auto' : ''}`}>
          ...
          <div className={`absolute left-1/2 bottom-6 md:bottom-10 -translate-x-1/2 ${viewMode === 'immersive' ? 'pointer-events-auto' : ''}`}>
            ...
            <div className="glass-panel px-2 py-2 rounded-2xl flex items-center gap-1 md:scale-100 transition-all duration-300">
              ...
              <button
                ...
                className="p-4 rounded-xl hover:bg-white/10 text-white/80 hover:text-white transition-all group flex flex-col items-center gap-1 w-20 active:scale-95 cursor-pointer touch-manipulation"
              >
              ...
              <button
                ...
                className={`p-4 rounded-xl transition-all group flex flex-col items-center gap-1 w-20 active:scale-95 cursor-pointer touch-manipulation ${showMinimap ? 'bg-white/10 text-white' : 'hover:bg-white/10 text-white/80 hover:text-white'}`}
              >
                <svg className="w-6 h-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {/* Right: Category/Meta */}
          <div className={`hidden md:block ${viewMode === 'immersive' ? 'pointer-events-auto' : ''}`}>
